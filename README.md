...
- **服务管理**：云服务定义和配置
- **云资源管理**：
  - 虚拟机全生命周期管理（创建/启动/停止/暂停/重启/删除）
  - **编辑实例**：修改名称、描述、安全组
  - **快照管理**：创建、回滚、删除快照
  - 配置调整（Resize）
  - 实时状态追踪、OpenStack 深度集成
  - 3D地球可视化、操作审计
- **账单管理**：租户账单查看、统计分析
- **订单管理**：订单查询和管理
...
#### 🚀 镜像上传革新
- ✅ **直接上传到 OpenStack**
  - 前端直接通过 Nginx 代理上传大文件到 Glance
  - 完全绕过 Django 后端，解决大文件 502 错误
  - 支持 10GB 以上镜像文件上传
- ✅ **实时上传进度**
  - 显示实时上传百分比（如：正在上传镜像 800.0 MB - 45%）
  - 上传按钮显示进度状态
- ✅ **取消上传功能**
  - 上传中可点击"取消上传"按钮
  - 自动删除已创建的镜像元数据
...
#### 🎨 镜像管理增强
- ✅ **完整格式支持**
  - 新增 VDI、VHD、AKI、AMI、ARI 格式
  - 与 OpenStack Horizon 格式列表完全一致
- ✅ **智能格式验证**
  - 自动检测文件扩展名是否与选择的格式匹配
  - 切换格式时自动重新验证已选文件
  - 错误提示包含正确的扩展名建议
...
#### 🐛 问题修复
- ✅ **时区显示正确**
  - 虚拟机创建时间使用 `moment.utc().local()` 转换
  - 运行时长计算使用 UTC 时间，不再多8小时
- ✅ **镜像列表刷新**
  - 上传/修改/删除后强制刷新镜像列表
  - 使用 `refreshImages(true)` 绕过缓存

#### ⚙️ 架构优化
- ✅ **Nginx 代理 Glance**
  - 添加 `/glance-proxy/` 路径代理到 OpenStack Glance
  - 禁用代理缓冲，支持大文件流式传输
  - 1小时上传超时配置
